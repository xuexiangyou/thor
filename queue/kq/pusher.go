...
import (
	...
	"github.com/xuexiangyou/thor/core/executors"
)
...
func newOptions(opts []PushOption) []executors.ChunkOption {
	...
	var chunOpts []executors.ChunkOption
	if options.chunkSize > 0 {
		chunOpts = append(chunOpts, executors.WithChunkBytes(options.chunkSize))
	}
	if options.flushInterval > 0 {
		chunOpts = append(chunOpts, executors.WithFlushInterval(options.flushInterval))
	}
	return chunOpts
}
